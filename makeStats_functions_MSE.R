...
makeStats1 <- function(df, species) {
  ...
  #calc MSE
  stats <-within(stats, MSE <- RMSE ^2) 
  #cal components of MSE
  stats <- within(stats, bias <- MB^2) 
  stats <- within(stats, var <- (sd.mod - r*sd.obs)^2 )
  stats <- within(stats, mMSE <- sd.obs^2 * (1-r^2)) 
  stats <- within(stats, MSE_sum <- bias + var + mMSE)
