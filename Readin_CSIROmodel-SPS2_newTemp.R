...
setwd(paste0("C:/Documents and Settings/eag873/My Documents/R_Model_Intercomparison/Model output/CSIRO_", folders[i], "/newTemp")) #renamed folder for consistency with the other campaigns
...
names(data)
#if (i ==1){
#data$prcp <- data$prcp *24 # I think the new output has the same issue as SPS2
#}
#if (i ==2){
...
data$data_source <- "C-CTM_newTemp"
data$campaign <- campaign[i]
#data <- subset(data, date >= start_date[i] & date <= end_date[i])

dataframe_name <- paste0("csiro_",campaign[i],"_newTemp") 
...
save(csiro_SPS2_newTemp, file = "CSIRO_model_output_SPS2_newTemp.RData")
...
SPS2_mod <- rbind.fill(csiro_SPS2,csiro_SPS2_newTemp)


a <- timeVariation(subset(SPS2_mod, site %in% "Westmead"), pollutant = "temp", group = "data_source", local.tz = "Etc/GMt-10", ci = FALSE)
print(a, subset = "hour")
...
SPS2_aq <- rbind.fill(SPS2_mod, OEH)

a2 <- timeVariation(subset(SPS2_aq, site %in% "Westmead"), pollutant = "temp", group = "data_source", local.tz = "Etc/GMt-10", ci = FALSE, cols = c("red", "blue", "black"))
